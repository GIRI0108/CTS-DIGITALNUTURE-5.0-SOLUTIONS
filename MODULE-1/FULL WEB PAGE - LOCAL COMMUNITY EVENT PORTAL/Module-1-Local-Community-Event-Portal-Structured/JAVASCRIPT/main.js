...
function registerUser(name = "", email = "", date = "", eventType = "", message = "") {
    ...
    if (selectedEvent == null) {
        selectedEvent = findEventByName(eventType);
    }

    if (selectedEvent == null) {
        console.error("Registration failed: Event not found");
        throw "Please select a valid event.";
    }

    if (selectedEvent.name != eventType) {
        console.error("Registration failed: Event mismatch");
        throw "Selected event does not match.";
    }
...
document.querySelector("#eventType").addEventListener("change", function() {
    ...
    const eventItem = findEventByName(eventName);

    if (eventItem != null && eventItem.checkAvailability()) {
        selectedEvent = eventItem;
        document.querySelector("#date").value = eventItem.date;
        clearFormErrors();
    } else {
        selectedEvent = null;
    }
...
eventForm.addEventListener("submit", function(event) {
    ...
    try {
        ...
        document.querySelector("#serverMessage").innerHTML = "";
        document.querySelector("#serverMessage").className = "";
